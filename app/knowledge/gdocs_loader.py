...
def export_doc_text(doc_id: str, fmt: str = "txt") -> str:
    ...
    try:
        ...
        log.info("GDOCS export response: doc_id=%s status=%s len=%s url=%s",
                 doc_id, r.status_code, len(r.text or ""), url)
        ...
        # если прилетел HTML (страница логина/ошибка) — считаем это фейлом
        if "<html" in low or "accounts.google.com" in low:
            log.error("GDOCS export returned HTML (likely private/blocked). url=%s preview=%r", url, text[:800])
